...
if($_GET['action']=="add" && !empty($_GET['name']) && !empty($_GET['amount'])){
	...
	if(intval($_GET['amount'])){
		...
		if($amount>0 && $amount<500){
			...
			if($res[3]>30){ // just accept the name if the likelyhood is at least 30% 
				...
				list($last_transfer)=mysqli_fetch_row($mysqli->query($request));
				if($last_transfer<$time-5*86400){
					$changed[$id]=1;
					if(!$mysqli->query( "INSERT INTO `aka_money` (`id` ,`value`, `date`) VALUES ('".$id."', '".$amount."', ".$time.")" )){
						exit("Exit 2");
					} else {
						echo "ok";
						//include('email.php');
	...
	list($ts)=mysqli_fetch_row($mysqli->query($request));	
