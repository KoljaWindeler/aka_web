...
if (isset($_GET['ansicht']))
{
	if ( $_GET['ansicht'] == 'Print') {
		...
		for ($a = 0; $a < count($daten); $a++) {
			...
			if (strlen($daten[$a][1]) > 21) {
				$daten[$a][1] = substr($daten[$a][1], 0, 19) . '...';
			} else {
				$daten[$a][1] = $daten[$a][1];
			};
			...
			$geld = round(floatval($daten[$a][8]) * 100) / 100;
		...
		if ($_SESSION['session_user_typ'] <> $aka_drinks_admin_state && $_SESSION['session_user_typ'] <> $aka_super_admin_state) {
