...
if(isset($_GET['remove'])){
	list($filename)=mysqli_fetch_row($mysqli->query("SELECT `Filename`FROM `aka_pro_list` WHERE `ID`='".$_GET['remove']."' LIMIT 0,1;"));
	if($mysqli->query("DELETE FROM `aka_pro_list` WHERE `ID`='".$_GET['remove']."' LIMIT 1;")){
		@unlink($filename);
...
elseif(isset($_GET['edit'])){
	list($beschr,$date,$filename)=mysqli_fetch_row($mysqli->query("SELECT `Bes`,`date`,`filename` FROM `aka_pro_list` WHERE `ID`='".$_GET['edit']."' LIMIT 0,1;"));
#################### upload tabelle #########################
...
elseif(isset($_POST['save'])){
	...
	if($mysqli->query("UPDATE `aka_pro_list` SET `Bes` = '".$_POST['beschr']."',`date`='".$date."' WHERE `ID` ='".$_POST['ID']."' LIMIT 1 ;")){
		echo'	<script type="text/javascript" />
		<!--
		location.href="index.php?'.SID.'&from='.$_GET['from'].'&to='.$_GET['to'].'";
		//-->
