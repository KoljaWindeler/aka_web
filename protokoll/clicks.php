...
if(mysql_num_rows(mysql_query("SELECT `clicks` FROM aka_pro_clicks WHERE `Filename`='".basename($db_filename)."' LIMIT 0,1;"))==0) { 
	mysql_query("INSERT INTO aka_pro_clicks (`ID` ,`Filename` ,`clicks`) VALUES (NULL , '".basename($db_filename)."', '1');");
	}
else {
	$clicks++;
	mysql_query("UPDATE `aka_pro_clicks` SET `clicks`='".$clicks."' WHERE `Filename`='".basename($db_filename)."';");
	};
