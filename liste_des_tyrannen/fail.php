...
if(!empty($_POST)){
    foreach( $_POST as $task_id => $index){
        if($index!="versaut"){
            ...
            mysqli_fetch_row(mysqli_db_query($db,"SELECT `FAIL`,`NUM_FAILED` FROM `aka_tasks_user` WHERE `id`=".$task_id[1].";",$verbindung));
            ...
            mysqli_fetch_row(mysqli_db_query($db,"SELECT `title`,`desc` FROM `aka_tasks` WHERE `id`=".$task_id[0].";",$verbindung));
