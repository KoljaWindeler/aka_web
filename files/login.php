...
if(!empty($_POST['pw_files']))
	{
	$msg = '<br /><font color="green" />Login, bitte warten...</font>';	
	if($_POST['pw_files']==$aka_pw) {
		//session_register('session_user_typ');
		$_SESSION['session_user_typ']=1; }
	elseif($_POST['pw_files']==$aka_files_pw) {
		//session_register('session_user_typ');
		$_SESSION['session_user_typ']=$aka_files_admin_state; }
	elseif($_POST['pw_files']==$aka_super_admin_pw) {
		//session_register('session_user_typ');
		$_SESSION['session_user_typ']=$aka_super_admin_state; };
		
	if($_POST['pw_files']==$aka_pw OR $_POST['pw_files']==$aka_files_pw OR $_POST['pw_files']==$aka_super_admin_pw) {
...
if(empty($_SESSION['session_key']))
	...
	htmlhead('AkAKraft Dateiupload Login','','');
	echo	'<table style="height: 100%; width: 100%" border="0" ><tr><td valign="middle" align="center"><img src="img/Logo.gif"><br /><br /><br /><br />
		<form method=post action="index.php?'.SID.'" name="login_files">';

	tab_go(600,'','center','Login');
	echo	'	<table align="center" width="400" style="height: 100%">	
			<tr><td width="40%">Passwort:</td><td align="right" width="40%"><INPUT type="password" name="pw_files" tabindex="2" class="inputbox" /></td>
