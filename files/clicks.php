...
if(mysqli_num_rows($mysqli->query("SELECT `clicks` FROM aka_pro_clicks WHERE `Filename`='".basename($db_filename)."' LIMIT 0,1;"))==0) {
	$mysqli->query("INSERT INTO `d00b711e`.`aka_pro_clicks` (`ID` ,`Filename` ,`clicks`) VALUES (NULL , '".basename($db_filename)."', '1');");
	}
else {
	$clicks++;
	$mysqli->query("UPDATE `d00b711e`.`aka_pro_clicks` SET `clicks`='".$clicks."' WHERE `Filename`='".basename($db_filename)."';");
	};
