...
if(mysqli_num_rows($mysqli->query("SELECT time,typ FROM `".$db_counter."` WHERE typ<>'9' AND time<$gestern")) > 200)
	...
	list($alte_ges) = mysqli_fetch_row($mysqli->query("SELECT time,typ FROM `".$db_counter."` WHERE typ='9'"));
...
list($grossezahl2) = mysqli_fetch_row($mysqli->query("SELECT time,typ FROM `".$db_counter."` WHERE typ='9'"));
$ges=$Allelines+$grossezahl2;
